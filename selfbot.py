...
async def on_message(mess):
    ...
    if mess.author.id == ZENITH_ID:
        if mess.server == client.get_server("94882524378968064") and mess.channel.id not in ["152757147288076297",
                                                                                             "170179130694828032",
                                                                                             "147153976687591424",
                                                                                             "200185170249252865",
                                                                                             "209609220084072450"]:
            toSend = await ascii_string(
                "[" + mess.channel.name + "] " + mess.author.name + ": -*0       " + mess.content + "\n")
                ...
                try:
                    ...
                    with open(PATHS["logs"] + str(channel.name) + ".csv", 'w', newline='') as myfile:
                        ...
                        for x in collections.Counter(messageCount).most_common():
                            y = (str(x[0]).encode('ascii', 'ignore').decode("utf-8"),
                                 str(x[1]).encode('ascii', 'ignore').decode("utf-8"))
        ...
        if "!getlogs" in mess.content:
            ...
            await client.send_message(mess.channel,
                                      "Log Retrieval complete after " + (datetime.utcnow() - time).strftime("%H:%m:%S"))
